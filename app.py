...
#         if not (0 <= inputs["Pressure_PSI"] <= 1000):
#             return jsonify({"error": "Pressure_PSI must be between 0 and 1000"}), 400
#         if not (0 <= inputs["FlowRate_GPM"] <= 100):
#             return jsonify({"error": "FlowRate_GPM must be between 0 and 100"}), 400
#         if not (-50 <= inputs["Temperature_Cel"] <= 100):
#             return jsonify({"error": "Temperature_Cel must be between -50 and 100"}), 400
#         if not (0 <= inputs["Moisture_Percent"] <= 100):
#             return jsonify({"error": "Moisture_Percent must be between 0 and 100"}), 400
#         if not (0 <= inputs["Acoustic_dB"] <= 200):
#             return jsonify({"error": "Acoustic_dB must be between 0 and 200"}), 400
...
#             "input_values": inputs,
#             "message": "High probability of a leak due to unusual readings!" if result else "All parameters seem normal."
#         }

#         user_id = USER_PHONE_NUMBER
#         if user_id in alert_timestamps and datetime.now() - alert_timestamps[user_id] < timedelta(minutes=5):
#             response["alert"] = "Rate limit: Alerts sent too recently. Try again later."
#             return jsonify(response)

#         if result and not DEBUG_MODE:
#             severity = "Critical" if probability > 0.75 else "Moderate"
#             sms_message = (
...
#             except Exception as e:
#                 response["sms_error"] = f"Failed to send SMS: {str(e)}"
...
#             except Exception as e:
#                 response["call_error"] = f"Failed to initiate call: {str(e)}"
