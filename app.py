...
# # Validate environment variables (excluding PUBLIC_URL since we'll automate it)
# required_env_vars = [
...
# demo_data = None
# try:
#     # Get the absolute path to demo_dataset.csv
...
# # Route for mobile access (for manual testing or dynamic generation if needed)
# @app.route('/mobile')
# def mobile():
#     qr_url = f"{public_url}/register"
#     qr = qrcode.QRCode(version=1, box_size=10, border=4)
...
#     # Attempt to terminate all ngrok processes, including those outside pyngrok
#     try:
#         # Kill pyngrok-managed tunnels
#         ngrok.kill()
#         logging.info("Terminated existing pyngrok tunnels.")
#     except Exception as e:
#         logging.warning(f"Failed to kill pyngrok tunnels: {e}")

#     # Try to kill any standalone ngrok processes (Windows-specific)
#     try:
...
#         public_url = "http://localhost:5000"  # Fallback to local URL
...
try:
    ...
    try:
        history = db.session.query(DetectionHistory).filter_by(leak=True).all()
        X = np.array([[h.probability] for h in history])
        y = np.array([1 if h.severity == 'Critical' else 0 for h in history])
        if len(X) > 0 and len(y) > 0:
            maintenance_model = LinearRegression().fit(X, y)
            joblib.dump(maintenance_model, MAINTENANCE_MODEL_PATH)
        else:
...
class WaterSaved(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    water_saved = db.Column(db.Float, nullable=False, default=0.0)
...
CHATBOT_RESPONSES = {
    ...
    "leak|help|problem": "Suspect a leak? Head to the Detect page to input sensor data. For urgent issues, call +91 7068460882.",
    "price|pricing|plan|cost": "We offer Basic ($0/month), Pro ($9.99/month), and Enterprise ($29.99/month) plans. See the Plans page for details.",
    "water saved|conservation": "LeakSentry has helped save {} liters of water so far! Run a detection to contribute more.",
    "support|contact|help me": "For support, email leaksentry@gmail.com or call +91 7068460882. I'm here for quick questions too!",
    "team|who made": "LeakSentry was built by Aarab, Kushagra, and Aayush. Check the Team page for more about them!",
    "feedback|review": "We'd love your feedback! Visit the Feedback page to share your thoughts.",
    "history|past detections": "You can view past leak detections on the History page.",
...
with app.app_context():
    db.create_all()
    # Initialize water_saved if not exists
    if not WaterSaved.query.first():
        initial_water = WaterSaved(water_saved=0.0)
...
@app.route("/")
def home():
    water_saved = WaterSaved.query.first().water_saved
    return render_template('home.html', water_saved=water_saved)
...
@login_required
def history():
    user_history = DetectionHistory.query.filter_by(user_id=current_user.id).all()
    return render_template('history.html', history=user_history)
...
def dashboard():
    try:
        ...
        total_detections = len(user_history)
        leaks_detected = sum(1 for item in user_history if item.leak)
        ...
        recent_probs = [item.probability for item in user_history[-5:] if item.leak]
        maintenance_risk = 0.0
        if recent_probs and len(recent_probs) > 0:
            maintenance_risk = float(maintenance_model.predict(np.array(recent_probs).reshape(-1, 1)).mean())
            maintenance_risk = max(0.0, min(1.0, maintenance_risk))
...
def create_checkout_session():
    try:
        ...
        session = stripe.checkout.Session.create(
            ...
            cancel_url=f"{base_url}/payment?plan={plan}",
        )
        return jsonify({"id": session.id})
    except Exception as e:
        logging.error(f"Checkout session error: {str(e)}")
...
def predict():
    try:
        ...
        if result:
            flow_rate = inputs["FlowRate_GPM"] * 3.78541  # Convert GPM to LPM
            minutes_saved = 60 if severity == "Critical" else 30 if severity == "Moderate" else 15
            water_saved_record = WaterSaved.query.first()
            water_saved_record.water_saved += flow_rate * minutes_saved
            response["water_saved"] = water_saved_record.water_saved
            db.session.commit()
        ...
        if result and not DEBUG_MODE:
            ...
            try:
                ...
                twilio_client.calls.create(
                    to=user_id,
                    from_=TWILIO_PHONE_NUMBER,
                    url=f"{base_url}/twiml?message={urllib.parse.quote(call_message)}"
                )
                logging.info("Call initiated successfully")
            except Exception as e:
                response["call_error"] = f"Call failed: {str(e)}"
            ...
            email_body = (
                ...
                f"Acoustic: {inputs['Acoustic_dB']} dB\n"
                f"Recommended action: {response['recommendation']}\n\n"
                f"View your detection history at {base_url}/history"
            )
...
def submit_feedback():
    try:
        ...
        logging.info(f"Feedback received from {name} ({email}): {feedback_text}")
        return jsonify({"message": "Thank you for your feedback!"}), 200
    except Exception as e:
        db.session.rollback()
        logging.error(f"Feedback submission error: {str(e)}")
...
def chat():
    try:
        ...
        for key, value in CHATBOT_RESPONSES.items():
            ...
            keywords = key.split("|")
            if any(keyword in user_message for keyword in keywords):
                response = value.format(water_saved) if "water saved" in key else value
                break
...
# Function to generate or reuse QR code
def generate_or_reuse_qr_code():
    ...
    if os.path.exists(qr_path):
        try:
            ...
            qr = qrcode.QRCode()
            qr.make_image(image_factory=qrcode.image.pil.PilImage)
            decoded_data = qr.decode(img)
            if decoded_data == qr_url:
                logging.info(f"Existing QR code is valid for URL: {qr_url}")
                return
        except Exception as e:
            logging.warning(f"Failed to validate existing QR code: {str(e)}. Generating new QR code.")
    ...
    os.makedirs(os.path.dirname(qr_path), exist_ok=True)
    img.save(qr_path)
    logging.info(f"New QR code generated and saved to {qr_path} with URL: {qr_url}")
...
if __name__ == "__main__":
    # Generate or reuse QR code on startup
    generate_or_reuse_qr_code()
    # Run Flask app
